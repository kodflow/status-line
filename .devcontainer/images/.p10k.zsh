...
() {
  ...
  # Dynamic segment override: generated by postStart.sh based on installed tools.
  # Replaces the static 47-segment RIGHT_PROMPT_ELEMENTS with only segments for
  # tools actually present in this container. Falls back to static list above
  # if the file doesn't exist yet (e.g., first shell before postStart completes).
  # Must not produce console output (instant prompt compatibility).
  [[ -f ~/.p10k-segments.zsh ]] && builtin source ~/.p10k-segments.zsh
